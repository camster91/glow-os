...
export default function MCPRegistry() {
  const [servers, setServers] = useState([
    { id: "1", name: "Mock Google Calendar", url: "http://127.0.0.1:8000/mcp/calendar", status: "active" }
  ])
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    setServers([...servers, { id: Date.now().toString(), name: newName, url: newUrl, status: "active" }])
    setNewName("")
    setNewUrl("")
  }
    ...
    <div className="flex flex-col items-center min-h-screen bg-zinc-50 dark:bg-black py-12 px-4">
      <div className="w-full max-w-3xl flex flex-col gap-8">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
          ...
          <form onSubmit={handleAdd} className="flex gap-4">
            ...
            <input 
              ...
              className="flex-1 bg-zinc-100 dark:bg-zinc-800 rounded-xl px-4 py-2 outline-none dark:text-white"
            />
            <button type="submit" className="bg-black dark:bg-white text-white dark:text-black px-6 py-2 rounded-xl font-medium flex items-center gap-2">
        ...
        <div className="flex flex-col gap-4">
          ...
          {servers.map(server => (
            <div key={server.id} className="flex items-center justify-between bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => handleRemove(server.id)} className="text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 p-2 rounded-xl transition-colors">
